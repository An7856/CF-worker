...
const MAX_EARLY_DATA_SIZE = 4096;
const CONNECT_PRIMARY_TIMEOUT_MS = 2000;
const CONNECT_BACKUP_TIMEOUT_MS = 1500;
const DNS_FALLBACK = 'https://cloudflare-dns.com/dns-query';

function cleanExpiredCaches() {
    const now = Date.now();
    for (const [key, time] of FAILED_IP_CACHE) {
        if (now - time > FAILED_TTL) FAILED_IP_CACHE.delete(key);
    }
    for (const [key, time] of DIRECT_FAIL_CACHE) {
        if (now - time > DIRECT_FAIL_TTL) DIRECT_FAIL_CACHE.delete(key);
    }
    for (const [key, { ts }] of dnsCache) {
        if (now - ts > DNS_CACHE_TTL) dnsCache.delete(key);
    }
}
...
async function handleLogin(req, env) {
    ...
    if (req.method === 'POST') {
        ...
        if (password === storedPassword) {
            ...
            if (loginAttempts.size >= 100) loginAttempts.delete(loginAttempts.keys().next().value);
...
async function saveConfigToKV(env, cfipArr, fdipArr, u = null, protocolCfg = null, cfCfg = null, proxyCfg = null, klp = null, newDyhd = null, newDypz = null, newStp = null, newDns = null) {
    ...
    if (!kv) return false;
    const unifiedConfig = {
        yx: cfipArr, fdc: fdipArr, uid: u || uid,
        dyhd: newDyhd || dyhd, dypz: newDypz || dypz,
        stp: newStp || stp,
        dns: newDns || dns,
...
async function queryDoH(domain, type, doh = cc?.dns || dns) {
    ...
    try {
        ...
        if (!res.ok) {
            const fallbackUrl = new URL(DNS_FALLBACK);
            fallbackUrl.searchParams.set('name', domain);
            fallbackUrl.searchParams.set('type', type);
            const res2 = await fetch(fallbackUrl, { headers: { 'Accept': 'application/dns-json' } });
            if (!res2.ok) return [];
            const data2 = await res2.json();
            const answer = (data2.Status === 0 && data2.Answer) ? data2.Answer : [];
            dnsCache.set(cacheKey, { data: answer, ts: Date.now() });
            return answer;
        }
        const data = await res.json();
        const answer = (data.Status === 0 && data.Answer) ? data.Answer : [];
        dnsCache.set(cacheKey, { data: answer, ts: Date.now() });
        return answer;
    } catch (e) { return []; }
}
...
async function universalConnectWithFailover(targetHost = 'www.google.com', targetPort = 443) {
    ...
    for (let i = 0; i < resolvedList.length; i++) {
        ...
        if (!FAILED_IP_CACHE.has(cacheKey)) {
             try {
                const socket = await connectWithTimeout(hostname, port, i === 0 ? CONNECT_PRIMARY_TIMEOUT_MS : CONNECT_BACKUP_TIMEOUT_MS);
                return { socket, server: { hostname, port, original: cacheKey } };
            } catch (e) {
                if (FAILED_IP_CACHE.size >= 256) FAILED_IP_CACHE.delete(FAILED_IP_CACHE.keys().next().value);
...
async function forwardTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, yourUUID, proxyCtx) {
    ...
    const establish3LayerConnection = async (data) => {
        ...
        if (proxyEnabled && proxyGlobal) {
            ...
            if (!sock) {
                if (DIRECT_FAIL_CACHE.size >= 256) DIRECT_FAIL_CACHE.delete(DIRECT_FAIL_CACHE.keys().next().value);
	...
	const readable = new ReadableStream({
		start(controller) {
			...
			try {
				...
				if (binaryString.length > MAX_EARLY_DATA_SIZE) {
					safeCloseWebSocket(serverSock);
					safeCloseStream();
					return;
				}
...
async function sub(req) {
    ...
    if (target === 'clash' || target === 'singbox') {
        ...
        const subApi = new URL(backend);
        subApi.searchParams.set('target', target);
        subApi.searchParams.set('url', rawSubUrl);
        subApi.searchParams.set('config', config);
        subApi.searchParams.set('emoji', 'true');
        subApi.searchParams.set('scv', 'false');
        try {
...
async function handleInit(req, env) {
    ...
    await sU(env, uuid);
    await saveConfigToKV(env, yx, fdc, uuid, null, null, null, loginPath, null, null, null, null);
    uid = uuid;
    const newToken = await signToken(env, Date.now() + SESSION_DURATION);
...
<body>
    <div class="admin-container">
        ...
        <form onsubmit="saveConfig(event)">
            ...
            <div class="card">
                <h3><i class="fas fa-shield-alt" style="color:#ec4899"></i> 协议与网络传输</h3>
                <div class="grid-2">
                    <div class="form-group">
                        <label>启用核心协议</label>
            ...
            <div class="card">
                ...
                <div class="form-group">
                    ...
                    <div class="help-text"><i class="fas fa-server"></i><span>默认使用 sky.rethinkdns.com，建议保持默认。</span></div>
                </div>
            </div>
